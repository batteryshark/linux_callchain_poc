...
#endif
    // Why the shit did I have it this way? RTLD_LOCAL | RTLD_LAZY
    void* lib_handle = dlopen(lib_name, RTLD_DEFAULT);
...
unsigned char MemUtils__patch_memory(void* target_addr, void* data_ptr, size_t data_len, unsigned char is_write, unsigned char is_exec) {
    ...
    if(!memcpy(target_addr, (unsigned char*)data_ptr, data_len)){return 0;}
...
unsigned char MemUtils__hotpatch_function(void* target_function_address, void* replacement_function_address, size_t target_original_bytes_size, struct HotPatch_Info* ctx, void** ptrampoline_address) {
    ...
    if(!MemUtils__alloc(ctx->target_original_bytes_size, 0, (void**)&ctx->target_original_bytes)){return 0;}

    // Create our Hook Stub - Backup instructions we're about to modify.
    if(!memcpy(hook_stub + HOTPATCH_ADDRESS_OFFSET, &ctx->replacement_function_address, sizeof(void*))){return 0;}
    if(!memcpy(ctx->target_original_bytes, ctx->target_function_address, ctx->target_original_bytes_size)){return 0;}

    // Allocate an executable page for our trampoline.
    if (!MemUtils__heap_alloc_exec_page(ptrampoline_address)) { return 0; }
    if(!*ptrampoline_address){return 0;}
    ctx->trampoline_address = *ptrampoline_address;
    // Write our stolen instructions to the head of the trampoline.
    if(!memcpy(ctx->trampoline_address, ctx->target_original_bytes, ctx->target_original_bytes_size)){{return 0;}}
    ...
    if(!memcpy(trampoline_stub + HOTPATCH_ADDRESS_OFFSET, &actual_return_address, sizeof(void*))){return 0;}

    // Write this to our return trampoline.
    if(!memcpy((unsigned char*)ctx->trampoline_address + ctx->target_original_bytes_size, trampoline_stub, sizeof(hotpatch_stub))){return 0;}
...
// Simple Wrapper for HotPatching
int HotPatch_patch(const char* module_name, const char* func_name, size_t target_original_bytes_size, void* replacement_function, void** original_function) {
    ...
    if (!MemUtils__get_function_address(module_name, func_name, &cf_address)) { return 0; }
    if (!MemUtils__hotpatch_function(cf_address, replacement_function, target_original_bytes_size, &ctx, original_function)) { return 0; }
    return 1;
}

int HotPatch_patchCET(const char* module_name, const char* func_name, size_t target_original_bytes_size, size_t flatten_size, void* replacement_function, void** original_function){
    void* cf_address = NULL;
    void* throwaway_function_addr = NULL;
    unsigned char flatten_bytes[16] = {0x00};
    memset(flatten_bytes,0x90,sizeof(flatten_bytes));
    if (!original_function) { original_function = &throwaway_function_addr; }
    struct HotPatch_Info ctx;
    memset(&ctx,0x00, sizeof(struct HotPatch_Info));
    if (!MemUtils__get_function_address(module_name, func_name, &cf_address)) { return 0; }
    if(!MemUtils__patch_memory(cf_address+4,flatten_bytes,flatten_size,0,1)){return 0;};
    if (!MemUtils__hotpatch_function(cf_address, replacement_function, target_original_bytes_size, &ctx, original_function)) { return 0; }
    return 1;
}
